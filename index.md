...
* 権限管理をしっかり行い、安全を確保し、その上でAIに好きにさせる
    ...
    * Fine-grained personal access tokensだけでもかなり頑張れる
    * AI専用アカウントも用意できるとさらに良い
* 権限管理は自動化を強く推奨
    * 確実に、素早く、漏れなく設定できる。手作業は楽なようで管理が大変
    * Terraform, API, etc, etc.
* 基本はモノレポ推奨
    * AIに横断で作業をさせるのが楽なので
* Git本体はもちろん、Issue, PRいずれも使うべき
    * AIに読ませるのが簡単
...
* 頻繁に使うコンテキストは英語化推奨
    * 日本語よりトークン効率が良いので。
        * 英語が苦痛で使わなくなったら本末転倒なので、「出来れば」レベルの工夫
    * 頻繁に使うコンテキストの代表はCLAUDE.md
* 機密情報とそうでもない情報が入り混じっていて分離もできない、というときは、問題ない個所だけ別の場所にクローンして使う
* 同じく全部AIに預けられないがAIをできるだけ使いたいワークフローがあるならヒューマンインザループで対応する
